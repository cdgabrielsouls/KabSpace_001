...
import { loadUserProfile } from '../data/profileRepository'
import { isSchoolEmail, schoolEmailMessage } from '../utils/emailValidator'

function getAuthErrorMessage(error: unknown) {
  if (error && typeof error === 'object' && 'code' in error && error.code === 'auth/unauthorized-domain') return 'This site is not authorized in Firebase. Add the current hostname under Firebase Authentication > Settings > Authorized domains.'
  return error instanceof Error ? error.message.replace('Firebase: ', '') : 'Authentication failed. Please try again.'
}

async function getPostLoginRoute(userId: string, isNewAccount: boolean) {
  if (isNewAccount) return '/onboarding'
  const profile = await loadUserProfile(userId)
  if (profile?.role === 'admin') return '/admin'
  if (profile?.role === 'student') return '/student'
  return profile?.role === 'professor' ? '/colleges' : '/onboarding'
}
...
export default function Login() {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    setBusy(true)
    try {
      const result = mode === 'signup' ? await createUserWithEmailAndPassword(auth, email.trim(), password) : await signInWithEmailAndPassword(auth, email.trim(), password)
      navigate(await getPostLoginRoute(result.user.uid, mode === 'signup'))
    } catch (authError) { setError(getAuthErrorMessage(authError)) } finally { setBusy(false) }
  }
