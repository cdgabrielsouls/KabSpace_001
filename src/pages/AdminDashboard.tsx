...
export default function AdminDashboard() {
  ...
  async function loadAdminData() {
    ...
    try {
      const [usersSnapshot, reservationsSnapshot] = await Promise.all([
        ...
        getDocs(query(collection(db, 'reservations'), where('status', 'in', ['PENDING', 'APPROVED']))),
      ])
  ...
  useEffect(() => {
    ...
    const unsubscribeReservations = onSnapshot(query(collection(db, 'reservations'), where('status', 'in', ['PENDING', 'APPROVED'])), (snapshot) => {
